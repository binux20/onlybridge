"""
OnlySQ -> OpenAI API Proxy (universal OpenAI-compatible).
For aider, Continue, Cline, Zed and any OpenAI-compatible client.
Emulates tool use via prompt injection (OnlySQ native tools are broken).

Start: uvicorn proxy_openaicompabilite:app --host 127.0.0.1 --port 7779
Client: base_url=http://127.0.0.1:7779/v1  api_key=any
"""
...
from backend.proxies._stats_db import log_request, tokens_from_messages, count_tokens
...
def build_tools_system(tools: list[dict]) -> str:
    ...
    lines = [
        ...
        "- Some tools may require specific fields marked with '*' - check the schema in the list below.",
...
async def openai_chat_completions(request: Request):
    ...
    if resp is None:
        log_request(source="openai_compat", model=model, prompt_tokens=prompt_tok, completion_tokens=0,
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=7779, log_level="info")
